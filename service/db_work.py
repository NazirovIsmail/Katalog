...
async def get_all_documents(conn):
    ...
    for document in all_documents:
        ...
        katalogs[katalog_parent_id]['inside'].append({'name':name, 'format':format, 'size':size, 'hash':hash})
    finish_katalog=[]
    for id in katalogs:
        katalog=katalogs[id]
        ...
        if parent_id==0:
            continue
        katalogs[parent_id]['inside'].append({'name':katalog['name'], 'inside':katalog['inside']})
    for id in katalogs:
        katalog=katalogs[id]
        if katalog['parent_id']==0:
            finish_katalog.append({'name':katalog['name'], 'inside':katalog['inside']})
...
async def get_document(conn,katalog_path,document_name):
    ...
    for katalog_name in split_katalog_path:
        values=await conn.fetch(
            '''SELECT id FROM katalog where name='%s' and parent_id='%s' '''%(katalog_name,parent_id)
        )
        if values==[]:
            return "wrong path"
